...
1) Исходные данные поступают во временную garbage-таблицу
2) Из нее с помощью процедуры информация после преобразования форматов загружается в 5 основных таблиц (TERMINALS, CLIENTS, ACCOUNTS, CARDS, TRANSACTIONS),
   при загрузке в которые информация о номере и дате обновления, количестве вставленных строк поступает в соответствующие им таблицы с метаданными (meta_таблицы).
3) По выделенным данным, хранящимся в основных таблицах с помощью представлений REPORT_OLD_PASSPORT и REPORT_OLD_CONTRACT строится витрина данных по подозрению на проведение мошеннических операций
   (совершение операций при просроченном паспорте/при недействующем договоре).
В витрине содержатся следующие поля:
- FRAUD_DT – дата и время наступления предполагаемого мошенничества. 
...
-- Создание временной garbage-таблицы
...
-- SELECT * FROM garbage_table;

/*SELECT CONVERT(CONCAT(SUBSTRING(trans_date, 7, 4), '-', SUBSTRING(trans_date, 4, 2), '-',
        SUBSTRING(trans_date, 1, 2), ' ', SUBSTRING(trans_date, 12, 8)), DATETIME)
    FROM garbage_table;

SELECT INSERT(amt, INSTR(amt, ','), 1, '.')
    FROM garbage_table;*/

-- DDL 5 таблиц с метаданными
...
-- Процедура вставки данных из временной garbage-таблицы в 5 основных таблицы и вставки мета-данных в соответствующие им 5 meta-таблиц
...
/*Представления REPORT_OLD_PASSPORT и REPORT_OLD_CONTRACT, с помощью которых строится витрина данных по подозрению на проведение мошеннических операций (совершение операций при просроченном паспорте/при недействующем договоре).
В витрине содержатся следующие поля:
- FRAUD_DT – дата и время наступления предполагаемого мошенничества. 
- PASSPORT – номер паспорта клиента 
- FIO – ФИО клиента 
...
CREATE OR REPLACE VIEW REPORT_OLD_PASSPORT AS 
    SELECT
        trans_date FRAUT_DT,
    ...
    FROM
        TRANSACTIONS t
        LEFT JOIN CARDS ca ON t.card_num = ca.card_num
        LEFT JOIN ACCOUNTS a ON ca.account_num = a.account_num
        LEFT JOIN CLIENTS cl ON a.client = cl.client_id
    WHERE passport_valid_to <= DATE(trans_date)
    ORDER BY FRAUT_DT;

CREATE OR REPLACE VIEW REPORT_OLD_CONTRACT AS
    SELECT
        trans_date FRAUT_DT,
        passport_num PASSPORT,
        CONCAT(last_name, ' ', first_name, ' ', patronymic) FIO,
    ...
    FROM
        ...
        LEFT JOIN ACCOUNTS a ON ca.account_num = a.account_num
        LEFT JOIN CLIENTS cl ON a.client = cl.client_id
    WHERE valid_to < DATE(trans_date)
    ORDER BY FRAUT_DT;

SELECT * FROM REPORT_OLD_PASSPORT
UNION
SELECT * FROM REPORT_OLD_CONTRACT;